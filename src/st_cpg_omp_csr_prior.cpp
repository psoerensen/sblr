...
inline void set_marker_effect_st_csr_prior(
  int i,
  double b_new,
  int d_new,
  const arma::rowvec& ww,
...
inline double residual_sse_st_csr_prior(
  int m,
  const arma::rowvec& b,
  const arma::rowvec& wy,
  const arma::rowvec& r,
...
inline int count_excluded_ld_friends_prior(
  int i,
  const arma::Row<int>& d,
  const LDLDFriendsPrior& friends
) {
 int n = 0;
...
inline int collect_ld_swap_candidates_prior(
  int m,
  const arma::Row<int>& d,
  const LDLDFriendsPrior& friends,
 ...
 for (int i = 0; i < m; ++i) {
  ...
  const int nf = count_excluded_ld_friends_prior(i, d, friends);
  if (nf > 0) {
...
inline double log_marker_prior_ratio_prior(
  double b_move,
  int j,
  int k,
  double vb_t,
...
inline bool attempt_ld_swap_st_csr_prior(
  int m,
  double vei,
  double yy,
  double vb_t,
 ...
 std::vector<int> candidates;
 std::vector<int> n_excluded;
 const int n_candidates = collect_ld_swap_candidates_prior(m, d, friends, candidates, n_excluded);
 ...
 const double sse_old = residual_sse_st_csr_prior(m, b, wy, r, yy);
 if (!std::isfinite(sse_old)) return false;

 const arma::rowvec r_old = r;

 set_marker_effect_st_csr_prior(j, 0.0, 0, ww, r, b, d, ld);
 set_marker_effect_st_csr_prior(k, b_j_old, 1, ww, r, b, d, ld);

 const double sse_new = residual_sse_st_csr_prior(m, b, wy, r, yy);
 bool accept = false;
 ...
 if (std::isfinite(sse_new)) {
  ...
  const int n_reverse_candidates =
   collect_ld_swap_candidates_prior(m, d, friends, reverse_candidates, reverse_n_excluded);
  ...
  if (n_reverse_candidates > 0 && n_reverse_friends > 0) {
   ...
   const double log_prior_ratio = log_marker_prior_ratio_prior(
...
sblr::core::CsrPriorBayesCExecutionResult stblr_cpg_omp_csr_prior_single(
 ...
 if (!std::isfinite(ld_swap_prob) || ld_swap_prob < 0.0 || ld_swap_prob > 1.0) {
  throw std::runtime_error("stblr_cpg_omp_csr_prior: ld_swap_prob must be in [0, 1].");
 }

 if (!std::isfinite(ld_swap_r2) || ld_swap_r2 < 0.0 || ld_swap_r2 > 1.0) {
  throw std::runtime_error("stblr_cpg_omp_csr_prior: ld_swap_r2 must be in [0, 1].");
 }

 if (ld_swap_max_friends <= 0) {
  throw std::runtime_error("stblr_cpg_omp_csr_prior: ld_swap_max_friends must be positive.");
 }

 if (ld_swap_moves < 0) {
  throw std::runtime_error("stblr_cpg_omp_csr_prior: ld_swap_moves must be non-negative.");
 }
 ...
 LDLDFriendsPrior ld_swap_friends;
 if (updateLDswap) {
  ld_swap_friends = build_ld_swap_friends_st_csr_prior(
   m,
   ld,
   xx,
   ld_swap_r2,
...
static Rcpp::List cpg_prior_chains_raw_v1(
 ...
 int nt,
 int nchains,
 const std::vector<std::vector<arma::mat>>& convergence_b,
 const std::vector<std::vector<arma::imat>>& convergence_d,
  ...
  for (int cc = 0; cc < nchains; ++cc) {
   ...
   chains[cc] = Rcpp::List::create(
    ...
    Rcpp::_["convergence_trace"] = Rcpp::List::create(
     ...
     Rcpp::_["b"] = convergence_b[static_cast<std::size_t>(cc)][static_cast<std::size_t>(t)],
     Rcpp::_["d"] = convergence_d[static_cast<std::size_t>(cc)][static_cast<std::size_t>(t)],
...
static Rcpp::List stblr_csr_prior_bayesc_result_to_raw(
 ...
 Rcpp::RObject chains = keep_chains ? Rcpp::RObject(cpg_prior_chains_raw_v1(
  raw, m, nt, nchains, convergence_b, convergence_d,
  convergence_markers)) : Rcpp::RObject(R_NilValue);
 ...
 Rcpp::List raw_out = Rcpp::List::create(
  ...
  Rcpp::_["diagnostics"] = Rcpp::List::create(
   Rcpp::_["nsamples"] = nsamples(Rcpp::_, 0),
   Rcpp::_["n_used"] = nsamples(Rcpp::_, 1),
...
Rcpp::List stblr_cpg_omp_csr_prior(
 ...
 std::vector<std::vector<arma::mat>> convergence_b_chains(
  static_cast<std::size_t>(nchains));
 std::vector<std::vector<arma::imat>> convergence_d_chains(
  static_cast<std::size_t>(nchains));
 std::vector<int> task_worker_ids(
  static_cast<std::size_t>(trait_count*nchains),0);
 std::vector<int> task_team_sizes(
  static_cast<std::size_t>(trait_count*nchains),1);
 ...
 for (int chain = 0; chain < nchains; ++chain) {
  ...
  sblr::core::CsrPriorBayesCExecutionResult execution_result =
   stblr_cpg_omp_csr_prior_single(
    ...
    pi_prior_b, ncores, chain_seed, phase3, chain, nchains,
    updateLDswap, ld_swap_prob, ld_swap_r2,
    ld_swap_max_friends, ld_swap_moves, convergence_markers_cpp,
   convergence_b, convergence_d
   );
  configured_workers=std::max(
   configured_workers,execution_result.configured_thread_count);
  for (int t=0;t<trait_count;++t) {
   const std::size_t task=static_cast<std::size_t>(t*nchains+chain);
   task_worker_ids[task]=execution_result.trait_worker_id[
    static_cast<std::size_t>(t)];
   task_team_sizes[task]=execution_result.actual_team_size;
  }
  convergence_b_chains[static_cast<std::size_t>(chain)]=
   execution_result.convergence_b;
  convergence_d_chains[static_cast<std::size_t>(chain)]=
   execution_result.convergence_d;
  const std::vector<std::vector<std::vector<double>>>& raw = execution_result.raw;
 ...
 if (return_chain_summaries) {
  ...
  return attach_phase3_workers(stblr_csr_prior_bayesc_result_to_raw(
   ...
   convergence_b_chains, convergence_d_chains, convergence_markers_cpp
  ));
 }
