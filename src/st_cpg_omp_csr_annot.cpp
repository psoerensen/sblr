...
std::vector<std::vector<std::vector<double>>> stblr_cpg_omp_csr_annot(
 ...
 std::vector<std::vector<std::vector<double>>> chain_eta_pi;
 std::vector<std::vector<std::vector<double>>> chain_eta_vb;
 ...
 for (int chain = 0; chain < nchains; ++chain) {
  ...
  if (chain == 0) {
   ...
   chain_eta_pi.resize(raw[18].size());
   chain_eta_vb.resize(raw[19].size());
  } else {
   for (std::size_t k = 0; k < out.size(); ++k) {
    for (std::size_t t = 0; t < out[k].size(); ++t) {
     for (std::size_t i = 0; i < out[k][t].size(); ++i) {
  ...
  if (keep_chains) {
   for (std::size_t t = 0; t < raw[0].size(); ++t) {
    ...
    chain_eta_pi[t].insert(chain_eta_pi[t].end(), raw[18][t].begin(), raw[18][t].end());
    chain_eta_vb[t].insert(chain_eta_vb[t].end(), raw[19][t].begin(), raw[19][t].end());
   }
  }
 }
 ...
 if (return_chain_summaries) {
  std::vector<std::vector<std::vector<double>>> extended(keep_chains ? 32 : 28);
  for (std::size_t k = 0; k < out.size(); ++k) extended[k] = out[k];
  for (int slot = 22; slot <= 27; ++slot) extended[static_cast<std::size_t>(slot)].resize(out[0].size());
  for (std::size_t t = 0; t < out[0].size(); ++t) {
   const std::size_t m_t = out[0][t].size();
   for (int slot = 22; slot <= 27; ++slot) extended[static_cast<std::size_t>(slot)][t].resize(m_t);
   for (std::size_t i = 0; i < m_t; ++i) {
    ...
    extended[22][t][i] = bm_sd;
    extended[23][t][i] = minv[0][t][i];
    extended[24][t][i] = maxv[0][t][i];
    extended[25][t][i] = dm_sd;
    extended[26][t][i] = minv[1][t][i];
    extended[27][t][i] = maxv[1][t][i];
   }
  }
  if (keep_chains) {
   extended[28] = chain_dm;
   extended[29] = chain_bm;
   extended[30] = chain_eta_pi;
   extended[31] = chain_eta_vb;
  }
  return extended;
 }
