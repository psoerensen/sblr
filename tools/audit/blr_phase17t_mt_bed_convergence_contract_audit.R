...
values <- c(
  CURRENT_RHAT_IMPLEMENTATION_COUNT = sum(grepl("rhat_rank|rhat_folded", c(public, native))),
  CURRENT_ESS_IMPLEMENTATION_COUNT = sum(grepl("ess_bulk|ess_tail", c(public, native))),
  CURRENT_MCSE_IMPLEMENTATION_COUNT = sum(grepl("mcse_mean", c(public, native))),
  ...
  BULK_ESS_EXPLICIT = has("initial positive sequence") && has("monotone sequence"),
  ...
  STAGED_IMPLEMENTATION_EXPLICIT = has("## 33. Phase 17U plan") && has("## 34. Phase 17V plan")
)
for (name in names(values)) cat(name, "=", values[[name]], "\n", sep = "")
stopifnot(values[["CURRENT_RHAT_IMPLEMENTATION_COUNT"]] == 0,
          values[["CURRENT_ESS_IMPLEMENTATION_COUNT"]] == 0,
          values[["CURRENT_MCSE_IMPLEMENTATION_COUNT"]] == 0,
          !values[["CURRENT_PUBLIC_CONVERGENCE_FORMAL"]],
          !values[["CURRENT_NATIVE_DIAGNOSTIC_TRACE_BUNDLE"]],
          all(values[c("CURRENT_CHAIN_VBS_TRACE", "CURRENT_CHAIN_VGS_TRACE",
                       "CURRENT_CHAIN_VES_TRACE")]),
          !any(values[c("CURRENT_MARKER_ITERATION_TRACES",
                        "CURRENT_FULL_COVARIANCE_ITERATION_TRACES",
                        "CURRENT_PI_ITERATION_TRACES")]),
          all(values[12:length(values)]))
