...
values <- c(
  ...
  public_convergence_warning_builder_count = grepl(".blr_convergence_warning_messages <-", engine, fixed = TRUE),
  public_convergence_warning_call_count = grepl("warning(raw$diagnostics$convergence$warning_messages[1L]", public, fixed = TRUE),
  convergence_memory_helper_count = grepl(".blr_convergence_memory_estimate <-", engine, fixed = TRUE),
