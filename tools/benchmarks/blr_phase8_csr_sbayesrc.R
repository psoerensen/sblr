# Canonical post-migration SBayesRC timing and completed-fit RSS baseline.
# Run manually from the package root; this script is never sourced by tests.
...
make_workload <- function(m = 2000L, nit = 120L, nburn = 30L) {
 ids <- paste0("m", seq_len(m)); prefix <- tempfile("phase8_sbayesrc_")
...
bench <- function(label, w, update_alpha, chains, cores, keep = FALSE, reps = 5L) {
 ...
 data.frame(configuration = label, elapsed = paste(elapsed, collapse = ","), mean = mean(elapsed),
  ...
  completed_fit_rss_mib = if (all(is.na(rss))) NA_real_ else max(rss, na.rm = TRUE), marker_count = w$stats$m,
...
cat("memory: whole-process RSS sampled after each completed fit; no interval peak sampler was available\n")
compiler <- if (is.null(R.version$compiler) || !nzchar(R.version$compiler)) "unavailable" else R.version$compiler
cat("environment:", R.version.string, "; sblr", as.character(utils::packageVersion("sblr")),
    "; platform", R.version$platform, "; compiler reported by R", compiler, "\n")
