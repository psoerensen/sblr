...
# comparisons. Requires external qgg data.
#
...
res <- test_read_bedfiles_to_packed_matrix(
 bedfiles = Glist$bedfiles[c(1, 1)],
 n = Glist$n,
 rows = 1:1000,
 cls = list(
  seq_along(Glist$rsids[[1]]),
  seq_along(Glist$rsids[[1]])
 )
)
str(res)

W <- getG(Glist, chr=1, rws=1:10, cls=1:5, scale=FALSE)

X <- test_read_bedfiles_to_dense_matrix(
 bedfiles = Glist$bedfiles[c(1,1)],
 n = Glist$n,
 rows = 1:10,
 cls = list(1:5, 1:5)
...
all.equal(X[, 1:5], X[, 6:10], check.attributes = FALSE)
