...
| ST BayesC fixed marker prior | scalar | binary | fixed_marker/global | fixed_marker | scalar | CSR | Migrated behind typed execution boundaries with borrowed `pi_marker`/`vb_multiplier`, callable core, typed result, one binding converter, and preserved wrapper aggregation | Public route/schema unchanged; exact references and runtime/memory checked; current trait restriction preserved | Ready for Phase 9C canonicalization |
...
| ST group BayesC | scalar | binary | group | group | scalar | CSR | Production unchanged; typed contracts and exact references established | Not migrated; seam documented | Group normalization and mutable state remain specific |
| ST learned annotation BayesC | scalar | binary | annot_logit | learned annotation scale | scalar | CSR | Production unchanged; typed contracts and exact references established | Not migrated; seam documented | Centered logistic/exponential MH policy remains specific |
