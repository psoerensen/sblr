...
| ST BayesC scheduled packed BED | scalar | binary | global_binary | unit | scalar | SNP-major packed BED | RNG ownership corrected with one chain-owned engine/normal/uniform state per logical chain; scheduler and decoder unchanged | Fit-local and worker-independent; post-correction references active; numerical-core migration not started | Phase 10A controls partially reusable; Phase 11A defective reference retained historically |
