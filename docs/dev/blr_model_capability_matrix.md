...
| ST BayesC scheduled packed BED | scalar | binary | global_binary | unit | scalar | SNP-major packed BED | Route/model, decoding, scheduler and RNG audit complete; production unchanged | Migration not started; worker-owned persistent distributions require correction | Phase 10A controls partially reusable after RNG correction; permanent fresh-process audit reference active |
