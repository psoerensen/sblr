...
| ST BayesC scheduled packed BED | scalar | binary | global_binary | unit | scalar | SNP-major packed BED | Corrected logical-chain RNG, typed borrowed per-chain context/result, callable per-chain core, typed aggregate result, one native aggregation path, and one named binding converter active | Fit-owned immutable genotype borrowed; task dispatch retained in adapter; Phase 11B references and fit-local/thread-independent reproducibility exact; public route/schema unchanged; migration complete and ready for canonicalization | Experimental single-chain route, BayesR and BayesRC unchanged |
