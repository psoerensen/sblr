...
| ST SBayesRC CSR | scalar | mixture | annot_probit_stick | component/maf_s | scalar | CSR | Migrated behind typed borrowed CSR/annotation context with operator-aware templated core, typed result, and one ordinary-CSR converter | Ordered-probit and alpha behavior preserved; public route/schema unchanged; exact references and runtime/memory validated | Ready for canonicalization; annotation-aware mixture reference |
