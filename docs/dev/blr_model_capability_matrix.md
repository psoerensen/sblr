...
| ST BayesC scheduled CSR | scalar | binary | global_binary | unit | scalar | CSR | Production unchanged; Phase 10A typed contracts, scheduler-state vocabulary, fresh-process references, and RNG-ownership audit active | Not migrated; explicit chain-owned distribution correction required | `static thread_local` normal state causes same-process/call-order and thread-assignment risk; Phase 10B remediation precedes migration |
