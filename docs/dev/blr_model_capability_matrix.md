...
| ST BayesC scheduled CSR | scalar | binary | global_binary | unit | scalar | CSR | Production scheduler unchanged; Phase 10B chain-owned engine/distributions active with fit-local and thread-assignment-independent reproducibility | RNG corrected; execution migration not started | Post-correction deterministic references active; scheduled BayesR remains unsupported |
