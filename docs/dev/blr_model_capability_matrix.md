...
| ST learned annotation BayesC | scalar | binary | annot_logit | learned annotation scale | scalar | CSR | Migrated typed implementation with borrowed annotation design, active learned policy, callable core, typed result, one binding converter, and one wrapper aggregation path | Migrated; public route/schema unchanged; exact references and runtime/completed-fit-RSS checks active; unsupported cases preserved; ready for canonicalization | Centered logistic/exponential MH policy, proposals, bounds, and update frequency remain specific |
