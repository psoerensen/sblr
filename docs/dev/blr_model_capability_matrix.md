...
| ST group BayesC | scalar | binary | group | group | scalar | CSR | Migrated behind typed execution boundaries with borrowed zero-based mapping, active group policy, callable core, typed result, one binding converter, and one wrapper aggregation path | Public route/schema and normalization semantics unchanged; exact references and runtime/completed-fit-RSS checked; unsupported cases preserved; ready for canonicalization | Group normalization and mutable state remain specific |
