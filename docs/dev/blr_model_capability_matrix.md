...
| ST BayesC scheduled packed BED | scalar | binary | global_binary | unit | scalar | SNP-major packed BED | Corrected logical-chain RNG, typed borrowed per-chain context, callable per-chain core, and typed per-chain result active | Fit-owned immutable genotype borrowed; task dispatch, cross-chain aggregation, and existing converter retained; Phase 11B references exact; migration in progress | Experimental single-chain route, BayesR and BayesRC unchanged |
