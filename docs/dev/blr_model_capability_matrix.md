...
| ST BayesC scheduled CSR | scalar | binary | global_binary | unit | scalar | CSR | Corrected chain-owned RNG, active scheduler contracts, typed borrowed context, callable core, typed result, one named binding converter, and one native aggregation path | Migration complete; public route/schema unchanged; ready for canonicalization | Post-correction references and fit-local/thread-assignment reproducibility exact; scheduled BayesR remains unsupported |
