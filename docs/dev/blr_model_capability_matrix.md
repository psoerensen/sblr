...
| ST BayesC scheduled CSR | scalar | binary | global_binary | unit | scalar | CSR | Corrected chain-owned RNG plus active Phase 10A scheduler contracts, typed borrowed execution context, callable numerical core, and typed result; existing inline converter retained | RNG corrected; typed execution migration active; converter closure pending | Post-correction references and fit-local/thread-assignment reproducibility exact; scheduled BayesR remains unsupported |
