...
| `learned_logistic` | \(\operatorname{logit}(\pi_j)=\operatorname{logit}(\pi)+\operatorname{center}(A_j\eta_\pi)\) | \(q_j=\exp\{\operatorname{clamp}(\operatorname{center}(A_j\eta_{vb}))\}\) | random-walk MH | hard probability and variance bounds | ST CSR BayesC only |
