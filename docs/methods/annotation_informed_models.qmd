...
::: {.callout-note}
This is a focused conceptual and historical page about deriving prior
information from first-stage posterior summaries. The canonical model
definitions, implemented capability table, preprocessing contract, LV model,
and proposed unified architecture are in
...
package APIs. Several MT sharing/covariance and direct in-sampler constructions
below are research proposals rather than implemented package features.
Executable package fits use the public fitters described in the
[model overview](../notes/model_overview.qmd).
:::
...
The general `sblr` annotation contract uses

$$
A\in\mathbb R^{M\times K},
$$

whose columns may contain binary annotations, continuous scores, or overlapping
binary and continuous information. See
[Annotation-Informed Priors](annotation_priors.qmd) for the canonical general
A-matrix, preprocessing, and identification contract.

For the set-based empirical-Bayes derivations on this page, it is useful to
specialize A to binary membership indicators:
...
This binary definition is an illustrative special case, not a restriction of
the broader package architecture. These annotation columns may overlap. For
example, a marker may be coding, close to a gene, and located inside a QTL
region at the same time.

This page follows the package-wide theory convention that lowercase v denotes
a scalar variance and uppercase V a variance-covariance matrix: ST uses
$v_b,v_g,v_e$, whereas MT uses $V_b,V_g,V_e$. Historical conceptual R code
snippets below may retain ordinary code variables such as `B`; those are not
the preferred mathematical symbols or proposed case-only API names.

The goal is to derive annotation-specific or marker-specific prior parameters such as:

$$
\pi_{k,t}, \qquad \sigma^2_{b,k,t}, \qquad V_{b,k}, \qquad \pi_{k,m},
$$
...
$$
\mathbf b_i \sim N(0, V_b).
$$

An annotation-informed extension uses annotation-specific covariance matrices:

$$
\mathbf b_i \mid a(i)=k \sim N(0, V_{b,k}).
$$

A simple estimate of $V_{b,k}$ is
...
$$
\mathbf b_i \sim \sum_m \pi_{i,m} N_m(0, V_{b,i})
$$
...
fitST <- fit_summary_model_conceptually(...)
...
This matrix can be used as the initial global $V_b$ matrix in the next MTBLR
run. The conceptual R snippet retains `B`/`B_init` as code-variable names; it
does not define the preferred mathematical notation or a public field rename.
...
# ------------------------------------------------------------
fit0 <- fit_summary_model_conceptually(...)
# or:
# fit0 <- fit_joint_summary_model_conceptually(...)
...
fitAnn_pip <- fit_annotation_summary_conceptually(
  y = S_pip,
  X = annot
)

fitAnn_vg <- fit_annotation_summary_conceptually(
  y = S_vg,
  X = annot
)

# Optional multi-trait annotation model.
fitAnn_mt <- fit_joint_annotation_summary_conceptually(
  Y = S_vg,
  X = annot,
  models = models
)
...
# ------------------------------------------------------------
fit1 <- fit_summary_model_conceptually(
  ...,
  pi_marker = pi_marker,
  vb_marker = vb_marker
)

fit1_mt <- fit_joint_summary_model_conceptually(
  ...,
  B = B_init,
...
Here $\pi_{it}$ is the prior inclusion probability for marker $i$ and trait $t$, $v_{b,it}$ is the marker-specific prior effect variance, $\pi_{im}$ is the marker-specific prior probability of MTBLR sharing model $m$, and $V_{b,i}$ is a marker-specific prior covariance matrix for multivariate marker effects.
...
$$
\mathbf b_i \mid M_i=m \sim N_m(0,V_b),
$$

where $N_m(0,V_b)$ denotes a multivariate normal distribution restricted to the active traits in model $m$.

An annotation-aware version can use marker-specific or annotation-specific covariance matrices:

$$
\mathbf b_i \mid M_i=m,A_i \sim N_m(0,V_{b,i}).
$$

A log-linear construction for trait-specific prior variances is:

$$
\log V_{b,i,tt} = \mu_{b,t} + A_i\eta_{b,t}.
$$
...
$$
D_i = \mathrm{diag}(\sqrt{V_{b,i,11}},\ldots,\sqrt{V_{b,i,TT}}),
$$

or use annotation-specific full covariance matrices $V_{b,k}$.
...
```r
fit <- fit_summary_model_conceptually(
  ...,
  pi_marker = pi_marker,
  vb_marker = vb_marker
)

fit_mt <- fit_joint_summary_model_conceptually(
  ...,
  pi_marker_model = pi_marker_model
)
```
...
These two extensions are simple, interpretable, and likely to be stable. Marker-specific prior variances $v_{b,it}$ and marker-specific covariance matrices $V_{b,i}$ are useful next steps, but they should be introduced with shrinkage and caps.
