...
#' @noRd
.mtblr_bed_legacy_covariance_hybrid <- function(
...
#' Fit corrected Cheng multi-trait BayesC from packed BED genotypes
#'
#' Fits the publicly supported common-sample Cheng MT-BayesC-Pi model using
...
#'   thinning interval.
#' @param seed User uint32 seed resolved through seed-contract version 1.
#' @param nchains Number of complete joint chains.
#' @param ncores Requested chain workers.
#' @param chain_seeds Optional signed-int32 or uint32-compatible base seed for
#'   each chain. These bases are resolved by seed-contract version 1.
#' @param keep_chains Retain compact chain-final records in the formatted fit.
#'   Raw v2 always preserves its explicit chain axes.
#' @param convergence Either `"core"` for the contracted observational capture
#'   or `"none"`. Disabling capture is zero-RNG.
#' @param convergence_control Reserved shared control. Phase 5B accepts only
#'   `NULL`; trace retention is controlled by `keep_traces`.
#' @param keep_traces Retain unthinned observational convergence traces.
#' @param memory_limit_bytes Incremental fit-allocation limit used by the
#'   pre-provider Phase 5A memory preflight. `NULL` or positive `Inf` disables
#'   the finite limit.
...
mtblr_bed <- function(
  ...
  if (!is.null(convergence_control)) {
    stop("Phase 5B mtblr_bed() requires convergence_control = NULL.",
         call. = FALSE)
  }
  ...
  .blr_format_cheng_mt_raw_v2(raw, keep_chains = keep_chains)
}
