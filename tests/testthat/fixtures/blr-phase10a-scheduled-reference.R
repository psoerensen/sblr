phase10a_starting_commit <- "ab18b9c"

phase10a_prefix <- function() {
  prefix <- tempfile("phase10a_scheduled_")
...
phase10a_run <- function(cfg) {
  captured <- NULL
  ns <- asNamespace("sblr")
  suppressMessages(trace(".as_stblr_fit", where=ns,
    tracer=quote(assign(".phase10a_raw_capture", raw, envir=.GlobalEnv)),
    print=FALSE))
  on.exit(suppressMessages(untrace(".as_stblr_fit", where=ns)), add=TRUE)
  fit <- sblr::stblr_csr(
    stats=phase10a_stats(), ld_prefix=phase10a_prefix(), scheduled=TRUE,
  ...
  captured <- get(".phase10a_raw_capture", envir=.GlobalEnv)
  rm(".phase10a_raw_capture", envir=.GlobalEnv)
  list(raw=phase10a_normalize(captured), fit=phase10a_normalize(fit))
}

phase10a_metadata <- function(name,cfg) list(
  starting_commit=phase10a_starting_commit, model="scheduled CSR BayesC",
