phase17b_mt_config <- function(id = 1L) {
  stopifnot(id %in% 1:3)
...
phase17b_mt_public_args <- function(config) {
  config$r_seed <- NULL
  config
}

phase17b_mt_native_arguments <- function(config) {
  seed_state <- config$r_seed
  args <- phase17b_mt_public_args(config)
...
phase17b_mt_native_raw <- function(config) {
  do.call(sblr:::mtblr, phase17b_mt_native_arguments(config))
}

phase17b_mt_capture <- function(id = 1L, formatted = TRUE) {
  config <- phase17b_mt_config(id)
