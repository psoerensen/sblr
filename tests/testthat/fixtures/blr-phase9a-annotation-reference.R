...
phase9a_run <- function(backend,cfg,raw=FALSE) {
 z<-phase9a_inputs(cfg$nt); common<-list(stats=z$stats,ld_prefix=phase9a_prefix(),pi_init=.3,pi_prior_mean=.3,pi_prior_strength=2,updateB=FALSE,updateE=FALSE,updatePi=FALSE,nit=6L,nburn=2L,nthin=1L,ncores=cfg$ncores,seed=909L,nchains=cfg$nchains,keep_chains=cfg$keep,chain_seeds=cfg$seeds,updateLDswap=FALSE)
 ...
 fit<-switch(backend,
  prior=do.call(sblr::stblr_csr_prior_annot,c(common,list(A=z$A,fixed_pi_marker=rep(list(c(.2,.35,.5,.65)),cfg$nt),fixed_vb_multiplier=rep(list(c(.7,1,1.4,2)),cfg$nt),use_pi_marker=TRUE,use_vb_multiplier=TRUE))),
  group=do.call(sblr::stblr_csr_group_annot,c(common,list(group=z$group,group_names=c("coding","background"),group_pi_init=c(.35,.2),group_vb_multiplier_init=c(1.4,.7),updateGroupVb=TRUE,normalize_group_vb=cfg$normalize))),
  annotation=do.call(sblr::stblr_csr_learn_annot,c(common,list(A=z$A,add_intercept=FALSE,standardize_annotations=FALSE,learn_pi_annot=cfg$learn_pi,learn_vb_annot=cfg$learn_vb,eta_pi_init=matrix(0,ncol(z$A),cfg$nt),eta_vb_init=matrix(0,ncol(z$A),cfg$nt),rw_sd_eta_pi=.02,rw_sd_eta_vb=.02,annot_update_every=cfg$every,pi_min=.01,pi_max=.9,vb_multiplier_min=.1,vb_multiplier_max=10))))
 if(raw){ out<-get(".phase9a_raw_capture",envir=.GlobalEnv); rm(".phase9a_raw_capture",envir=.GlobalEnv); out } else fit
}
phase9a_normalize <- function(x){ if(!is.list(x)) return(x); for(nm in names(x)) if(nm%in%c("seconds_mean","seconds_max")) x[[nm]][]<-0 else if(nm=="ld_prefix") x[[nm]]<-"<fixture>" else x[[nm]]<-phase9a_normalize(x[[nm]]); x }
