...
test_that("Phase 5A protects BayesC, block-eigen, and namespace sources", {
  ...
  expect_equal(source_match_count("for (int it = 0; it < trace_len",paste(src,core),fixed=TRUE),1L)
  expect_match(src,"run_csr_bayesr(execution_context)",fixed=TRUE)
  expect_match(src,"stblr_csr_bayesr_result_to_raw",fixed=TRUE)
