phase9f3_path <- function(...) {
  path <- file.path(...)
  if (file.exists(path)) path else file.path("..", "..", ...)
}

source(phase9f3_path(
  "tests", "testthat", "fixtures", "blr-phase9a-annotation-reference.R"
))

phase9f3_active <- function(lines, text) {
  sum(grepl(text, lines, fixed = TRUE) & !grepl("^\\s*//", lines))
}

test_that("Phase 9F3 has one core, converter, and aggregation path", {
  source_lines <- readLines(phase9f3_path("src", "st_cpg_omp_csr_annot.cpp"), warn = FALSE)
  core_lines <- readLines(phase9f3_path(
    "src", "blr_csr_learned_annotation_bayesc_core_impl.h"
  ), warn = FALSE)
  type_lines <- readLines(phase9f3_path(
  ...
  all_sources <- list.files(phase9f3_path("src"), pattern = "\\.(cpp|h)$", full.names = TRUE)
  ...
  expect_equal(phase9f3_active(core_lines, "run_csr_learned_annotation_bayesc("), 1L)
  expect_equal(phase9f3_active(core_lines, "for (int isort = 0; isort < m; ++isort)"), 1L)
  expect_equal(phase9f3_active(source_lines,
  ...
  expect_match(source_text, "execution_result.raw=std::move(extended)", fixed = TRUE)
  expect_match(source_text, "execution_result.raw=std::move(out)", fixed = TRUE)
...
test_that("Phase 9F3 preserves learned policy inside the numerical core", {
  core_text <- paste(readLines(phase9f3_path(
    "src", "blr_csr_learned_annotation_bayesc_core_impl.h"
  ), warn = FALSE), collapse = "\n")
  source_text <- paste(readLines(phase9f3_path(
  ...
  adapter_lines <- readLines(phase9f3_path(
    "src", "st_cpg_omp_csr_annot.cpp"
  ), warn = FALSE)
...
test_that("Phase 9F3 learned-annotation references remain exact", {
  expect_length(phase9a_configs$annotation, 3L)
  for (name in names(phase9a_configs$annotation)) {
    reference <- readRDS(phase9f3_path(
...
test_that("Phase 9F3 learned-annotation route remains reproducible", {
...
test_that("Phase 9F3 protects public and unrelated backends", {
  protected <- c(
  ...
  actual <- unname(tools::md5sum(vapply(names(protected), phase9f3_path, character(1))))
