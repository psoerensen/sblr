...
test_that("Phase 17P rejects unsupported or malformed public input", {
  ...
  expect_error(call(method = "bayesrc"), "annotations is required")
...
test_that("Phase 17P public architecture is singular and protected", {
  ...
  expected_formals <- c(
    ...
    "component", "annotations", "add_intercept",
  ...
  expect_equal(sum(grepl("native_result <- do.call(native_route, native_arguments)",
                         source, fixed = TRUE)), 1L)
  expect_true(any(grepl("mtblr_bed_chains_internal", source,
                         fixed = TRUE)))
  expect_true(any(grepl("mtblr_bed_convergence_trace_internal", source,
                         fixed = TRUE)))
