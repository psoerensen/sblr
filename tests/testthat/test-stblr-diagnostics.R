fake_stblr_fit <- function() {
  ...
  list(
    ...
    ves = matrix(rnorm(40, 0.4, 0.02), ncol = 1,
                 dimnames = list(NULL, trait)),
    pi_trace = matrix(runif(40, 0.01, 0.03), ncol = 1,
                 dimnames = list(NULL, trait)),
...
test_that("legacy convergence helper is absent from the public API", {
  expect_false("check_stblr_convergence" %in% getNamespaceExports("sblr"))
})
