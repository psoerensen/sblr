...
test_that("public source freezes guards, conditions, and distinct counts", {
  ...
  expect_source_count('#include "blr_mt_default_core_impl.h"',
    phase17c_text("src/mtblr.cpp"), 1L)
  expect_source_count("if (execution.updateB) {", execution, 1L)
  expect_source_count("sampleBset(nt, m, nub, B", execution, 1L)
  expect_source_count("sampleB_latent(nt, m, nub, B", execution, 1L)
  expect_source_count("if(execution.updateB && method==4)", execution, 1L)
  expect_source_count("it >= execution.nburn", execution, 10L)
  expect_source_count("(it - execution.nburn) % execution.nthin", execution, 2L)
