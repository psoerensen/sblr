...
test_that("group annotation CSR BayesC is backward compatible without LD-swap", {
  ...
  expect_annotation_group_ldswap_diagnostics(fit, expected_attempted = NULL)
})
...
test_that("old group annotation wrapper is not exported", {
  expect_false("stblr_csr_group_annot" %in% getNamespaceExports("sblr"))
})
