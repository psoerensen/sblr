test_that("public MT CSR fit uses named raw and matches the internal route", {
  ...
  expect_equal(unname(fit$pi_final), legacy[[17L]][[1L]], tolerance = 1e-12)
  expect_equal(unname(fit$pi_mean), legacy[[18L]][[1L]], tolerance = 1e-12)
...
test_that("explicit seed and stable metadata are reproducible", {
  ...
  expect_identical(a$data$alignment$per_trait$allele_status, rep("exact", 2L))
  expect_true(all(c("trait_id", "study_id", "ancestry", "population", "ld_reference",
                    "sample_size", "ld_prefix", "ld_sharing_mode") %in% names(a$data$trait_metadata)))
})
...
test_that("public route remains singular and research routes are absent", {
  ...
  expect_match(r, "mtblr_csr_chains_raw_internal", fixed=TRUE)
  ...
  expect_identical(source_match_count("mtblr_csr_chains_raw_internal(", cpp, fixed=TRUE), 1L)
})
